...
class Temporal(Module):
    def __init__(self, input_size,out_size,dropout):
        super(Temporal, self).__init__()
        self.dropout = nn.Dropout(dropout)
        self.conv_1 = nn.Sequential(
            nn.Conv1d(in_channels=input_size, out_channels=out_size, kernel_size=1, padding=0),
            nn.ReLU(),
        )
    def forward(self, x):
        x = x.permute(0, 2, 1)
...
#         if self.manifold.name == args.manifold1:
#             self.cls = HypLinear(self.manifold, self.input_dim, self.output_dim, args.c, args.dropout, args.bias)
#         else:
#             self.cls = nn.Linear(self.input_dim, self.output_dim, self.use_bias)
#
#     def forward(self, x):
...
class Model(nn.Module):
    def __init__(self, args, flag):
        ...
        # self.selfatt = Transformer(128, 1, 4, 32, 128, dropout=0.5)
        ...
        # self.Temporal1 = Temporal(args.c_in_feat, args.c_hidden, args.dropout)
        # self.Temporal2 = Temporal(args.c_hidden, args.c_out_feat, args.dropout)
    ...
    def forward(self, inputs, seq_len):
        ...
        # frame_prob =  self.sigmoid(self.HyperCLS(out).reshape((b, n, -1)).mean(1))
        frame_prob = self.HyperCLS(out).reshape((b, n, -1)).mean(1)


        if self.flag == 'Train':
            batch_size = int(b // 2)
            nor_out = out[:batch_size]
            abn_out = out[batch_size:]
            nor_score = frame_prob[:batch_size]
            abn_score = frame_prob[batch_size:]
            _, A1_index = torch.topk(nor_score, t // 16 + 1, dim=-1)
            A1_index = A1_index.unsqueeze(2).expand([-1, -1, out_x.size(-1)])
            anchor_abn_x = torch.gather(nor_out, 1, A1_index).mean(1).reshape(b // 2, n, -1).mean(1)

            _, P_index = torch.topk(nor_score, t // 16 + 1, dim=-1)
            positivte_nx = torch.gather(abn_out, 1, P_index.unsqueeze(2).expand([-1, -1, out_y.size(-1)])).mean(
                1).reshape(b // 2, n, -1).mean(1)

            _, N_index = torch.topk(abn_score, t // 16 + 1, dim=-1)
            negative_nor_x = torch.gather(abn_out, 1, N_index.unsqueeze(2).expand([-1, -1, out_x.size(-1)])).mean(
                1).reshape(b // 2, n, -1).mean(1)

            triplet_margin_loss = triplet_loss(norm(anchor_abn_x), norm(positivte_nx), norm(negative_nor_x), 1)
